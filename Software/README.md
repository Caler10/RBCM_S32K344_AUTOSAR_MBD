# WUTE-MQTT-Dashboard

<BR>使用Qt6.5.7编译

<BR>已经测试windows平台编译完成

## 部分界面展示：

![WUTE-MQTT-DashBoard1.png](WUTE-MQTT-DashBoard1.png)

![WUTE-MQTT-DashBoard2.png](WUTE-MQTT-DashBoard2.png)

![WUTE-MQTT-DashBoard3.png](WUTE-MQTT-DashBoard3.png)

![WUTE-MQTT-DashBoard4.png](WUTE-MQTT-DashBoard4.png)

## 开发日志

2024.12.3：WUTE_MQTT_DashBoard_v1.0.0.20241203_base.exe打包发布，完成上位机前端基础界面开发

2025.1.10：WUTE_MQTT_DashBoard_beta_20250110.exe打包发布，可成功连接MQTT服务器订阅接收topic数据

2025.1.27：实现MQTT数据接收到前端的实时显示，解决了C++后端至qml前端数据通信异常的问题
