...
#define FLEXPWM_IP_HWACCESS_SW_MAJOR_VERSION             4
...
{
    #ifdef FLEXPWM_AE_IP_HW
    return ((Base->SUB[SubModule].CTRL2 & FLEXPWM_CTRL2_FORCE_SEL_MASK) >> FLEXPWM_CTRL2_FORCE_SEL_SHIFT);
    #else
...
static inline void FlexPwm_Ip_SetCompareIntHw(FlexPwm_Ip_HwAddrType *const Base, uint8 SubModule, uint8 Val)
{
    ...
    #ifdef FLEXPWM_AE_IP_HW
    u16Register = Base->SUB[SubModule].INTEN;
    Base->SUB[SubModule].INTEN |= (u16Register & ~(Val)) | FLEXPWM_INTEN_CMPIE(Val);
    #else
    u16Register = Base->SM[SubModule].INTEN;
    Base->SM[SubModule].INTEN  |= (u16Register & ~((uint16)FLEXPWM_INTEN_CMPIE_MASK)) | FLEXPWM_INTEN_CMPIE(Val);
    #endif
}
