...
#define ADC_SAR_IP_SW_MAJOR_VERSION               4
 ...
 *
 */
#define ADC_SAR_IP_WDG_HIGH_FLAG                  ((uint32)1UL << 1U)
#define ADC_SAR_IP_WDG_LOW_FLAG                   ((uint32)1UL << 0U)
 ...
 * @param[in] u32Instance - ADC instance number
 * @param[in] pChainType - conversion chain (Normal or Injected)
 * @param[in] u32ChnIdx - channel index(channel number in the RM)
 * @return void
 */
...
Adc_Sar_Ip_StatusType Adc_Sar_Ip_Powerdown(const uint32 u32Instance);
     ...
     ((ADC_SAR_IP_HAS_CTU == 1U) && (STD_ON == ADC_SAR_IP_EOCTU_ENABLED)) \
    )
/*!
