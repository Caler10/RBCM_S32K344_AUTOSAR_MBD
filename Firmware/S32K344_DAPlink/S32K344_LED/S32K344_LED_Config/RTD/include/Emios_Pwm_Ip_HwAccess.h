...
#define EMIOS_PWM_IP_HWACCESS_SW_MAJOR_VERSION             4
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Emios_Pwm_Ip_GetChannelIndex
 * Description   : Get the value of channel index corresponding the specific hw channel
 *END**************************************************************************/
static inline uint8 Emios_Pwm_Ip_GetChannelIndex(uint8  Instance, 
                                                 uint8  Channel)
{
    uint8 channelIdx = 0;

    if ((Instance < EMIOS_PWM_IP_INSTANCE_COUNT) && (Channel < EMIOS_PWM_IP_CHANNEL_COUNT))
    {
        channelIdx = eMios_Pwm_Ip_IndexInChState[Instance][Channel];
    }
    else
    {
