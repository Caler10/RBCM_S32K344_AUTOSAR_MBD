...
#define FLEXPWM_IP_IRQ_SW_MAJOR_VERSION_C               4
...
void FlexPwm_Ip_ProcessCompareIrq(uint8 Instance, uint8 SubModule)
{
    ...
    else
    {
        ...
        if (0x00U != Status)
        {
            ...
            {
                UniqueSub = (uint8)((SubModule * FLEXPWM_IP_NUMCHANNELS_PER_SUBMODULE) + ChannelIdx);
                ChInterruptEnStatus = ((Status >> (2U * ChannelIdx)) & 3U);
