...
==================================================================================================*/
#define SCHM_AE_AR_RELEASE_MAJOR_VERSION_C     4
#define SCHM_AE_AR_RELEASE_MINOR_VERSION_C     7
#define SCHM_AE_AR_RELEASE_REVISION_VERSION_C  0
#define SCHM_AE_SW_MAJOR_VERSION_C             4
#define SCHM_AE_SW_MINOR_VERSION_C             0
#define SCHM_AE_SW_PATCH_VERSION_C             0
...
static volatile uint32 msr_AE_EXCLUSIVE_AREA_00[NUMBER_OF_CORES];
static volatile uint32 reentry_guard_AE_EXCLUSIVE_AREA_00[NUMBER_OF_CORES];
...
#ifdef MCAL_PLATFORM_ARM
#if (MCAL_PLATFORM_ARM == MCAL_ARM_AARCH64)
ASM_KEYWORD uint32 Ae_schm_read_msr(void)
{
    mrs x0, S3_3_c4_c2_1
}
#elif  (MCAL_PLATFORM_ARM == MCAL_ARM_RARCH)
...
#ifdef MCAL_PLATFORM_S12
    #define Ae_schm_read_msr()  ASM_KEYWORD("tfr ccr, d6")
#else
    #define Ae_schm_read_msr() ASM_KEYWORD("mfmsr r3")
#endif
...
void SchM_Enter_Ae_AE_EXCLUSIVE_AREA_00(void)
{
    uint32 msr;
    uint32 u32CoreId = (uint32)OsIf_GetCoreID();

    if(0UL == reentry_guard_AE_EXCLUSIVE_AREA_00[u32CoreId])
    {
#if (defined MCAL_ENABLE_USER_MODE_SUPPORT)
        msr = OsIf_Trusted_Call_Return(Ae_schm_read_msr);
#else
        msr = Ae_schm_read_msr();  /*read MSR (to store interrupts state)*/
    ...
    reentry_guard_AE_EXCLUSIVE_AREA_00[u32CoreId]++;
}

void SchM_Exit_Ae_AE_EXCLUSIVE_AREA_00(void)
{
    uint32 u32CoreId = (uint32)OsIf_GetCoreID();

    reentry_guard_AE_EXCLUSIVE_AREA_00[u32CoreId]--;
    if ((ISR_ON(msr_AE_EXCLUSIVE_AREA_00[u32CoreId]))&&(0UL == reentry_guard_AE_EXCLUSIVE_AREA_00[u32CoreId]))         /*if interrupts were enabled*/
    {
...
@remarks Implements 
*/
void SchM_Check_ae(void)
{
    uint32 u32CoreId = (uint32)OsIf_GetCoreID();

    EU_ASSERT(0UL == reentry_guard_AE_EXCLUSIVE_AREA_00[u32CoreId]);
    reentry_guard_AE_EXCLUSIVE_AREA_00[u32CoreId] = 0UL; /*reset reentry_guard_AE_EXCLUSIVE_AREA_00 for the next test in the suite*/
