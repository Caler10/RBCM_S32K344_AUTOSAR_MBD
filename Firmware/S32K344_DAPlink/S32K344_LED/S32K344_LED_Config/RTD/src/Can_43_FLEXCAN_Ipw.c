...
#define CAN_43_FLEXCAN_IPW_SW_MAJOR_VERSION_C             4
...
static void Can_Ipw_InitLegacyFifoFilter(const Can_43_FLEXCAN_ControllerConfigType * Can_pController, const Can_43_FLEXCAN_HwObjectConfigType * Can_pHwObject)
{
    ...
    for (u8FilterIdx=0U; ((u8FilterIdx < u8FiFoFilterIndvCount) && (u8FilterIdx < Can_pHwObject->Can_u8HwFilterCount)); u8FilterIdx++)
    {
            if (CAN_43_FLEXCAN_LEGACY_FIFO_FORMAT_A == Can_pController->Can_eLegacyAcceptanceMode)
            {
                ...
                if (Can_pHwObject->Can_IdMessage != CAN_43_FLEXCAN_STANDARD)
                {
...
static void Can_Ipw_InitRx(const Can_43_FLEXCAN_ControllerConfigType * Can_pController)
{
    ...
    for (u8HwObjRefIdx = 0U; u8HwObjRefIdx < Can_pController->Can_u8HwObjectRefCount; u8HwObjRefIdx++)
    {
        ...
        if (CAN_43_FLEXCAN_RX_LEGACY_FIFO == Can_pHwObject->Can_eReceiveType)
        {
            Can_Ipw_InitLegacyFifoFilter(Can_pController, Can_pHwObject);
        }
#if (CAN_43_FLEXCAN_ENHANCED_FIFO_ENABLED == STD_ON)
        if ((CAN_43_FLEXCAN_RX_ENHANCED_FIFO == Can_pHwObject->Can_eReceiveType) && ((Can_pHwObject->Can_u8HwFilterCount) != 0U))
        {
    ...
    */
    ...
    )
    {
        switch (mbType)
        {
            ...
            {
                Can_43_FLEXCAN_Ipw_ProcessRxEnhance(Can_pControllerConfig, Can_pControllerConfig->Can_ppHwObject[0U], u8MbIdx);
                break;
            }
        #endif
            default:
