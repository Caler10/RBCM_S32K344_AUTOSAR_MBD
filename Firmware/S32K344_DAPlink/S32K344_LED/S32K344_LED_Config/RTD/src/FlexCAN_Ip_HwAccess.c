 ...
 *
 *END**************************************************************************/
void FlexCAN_SetErrIntCmd(FLEXCAN_Type * base, Flexcan_Ip_ErrorIntMaskType errType, boolean enable)
{
                            ...
                            )
{
    ...
    uint32 NumOfFilters = (((base->CTRL2) & FLEXCAN_CTRL2_RFFN_MASK) >> FLEXCAN_CTRL2_RFFN_SHIFT);
...
void FlexCAN_ReadEnhancedRxFifo(const uint8 u8Instance, Flexcan_Ip_MsgBuffType * rxFifo)
{
    ...
    Flexcan_Ip_MsgBuffType * ptrRxFifo = NULL_PTR;
#endif
    ...
    if (((*Flexcan_Mb) & FLEXCAN_IP_CS_RTR_MASK) != 0U)
    {
        can_real_payload = 0U;
    }
    ...
    filterElement = (((*Flexcan_Mb) & FLEXCAN_IP_CS_IDE_MASK) == 0U) ? ((uint8)(((base->ERFCR) & FLEXCAN_ERFCR_NEXIF_MASK) >> FLEXCAN_ERFCR_NEXIF_SHIFT) + idHitaux) : idHitaux;
    ...
    FlexCAN_Ip_IdHit_Callout(u8Instance, idHitVal, &ptrRxFifo);
    if (NULL_PTR != ptrRxFifo)
    {
        /* in this case, original pointer rxFifo is updated by user pointer ptrRxFifo after calling callout */
        rxFifo = ptrRxFifo;
    }
...
void FlexCAN_SetEnhancedRxFifoFilter(const uint8 u8Instance, const Flexcan_Ip_EnhancedIdTableType * idFilterTable)
{
    ...
    uint8 NumOfEnhancedFilter = (uint8)(((base->ERFCR) & FLEXCAN_ERFCR_NFE_MASK) >> FLEXCAN_ERFCR_NFE_SHIFT);
    uint8 NumOfExtIDFilter = (uint8)(((base->ERFCR) & FLEXCAN_ERFCR_NEXIF_MASK) >> FLEXCAN_ERFCR_NEXIF_SHIFT);
...
#endif
{
    ...
    TimestampIns = (uint16)instance + FLEXCAN_IP_TIMESTAMP_TBS_START_INDEX;
