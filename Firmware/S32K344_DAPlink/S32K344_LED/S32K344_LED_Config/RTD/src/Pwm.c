...
#define PWM_SW_MAJOR_VERSION_C                  4
...
#if (defined(PWM_GETOUTPUTSTATE_PLAUSABILITY) && (PWM_GETOUTPUTSTATE_PLAUSABILITY == STD_ON))
static inline Std_ReturnType Pwm_ValidateGetOutputState(uint32            CoreId,
                                                        Pwm_ChannelType   ChannelNumber);
#endif /* (defined(PWM_GETOUTPUTSTATE_PLAUSABILITY) && (PWM_GETOUTPUTSTATE_PLAUSABILITY == STD_ON)) */
                                                        ...
                                                        Pwm_ChannelType   ChannelNumber)
{
    ...
    RetVal = Pwm_Ipw_ValidateGetOutputState(&((*Pwm_aState[CoreId].PwmChannelConfigs)[ChannelNumber].IpwChannelCfg));
...
#if (defined(PWM_GETOUTPUTSTATE_PLAUSABILITY) && (PWM_GETOUTPUTSTATE_PLAUSABILITY == STD_ON) && (PWM_PARAM_CHECK == STD_ON))
        if((Std_ReturnType)E_OK == Pwm_ValidateGetOutputState(CoreId, PartitionChannelIdx))
        {
#endif /* PWM_GETOUTPUTSTATE_PLAUSABILITY && PWM_PARAM_CHECK */
...
#if (PWM_VALIDATE_CHANNEL_CONFIG_CALL == STD_ON)
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(CoreId, ChannelNumber, PWM_SETTRIGGERDELAY_ID))
    {
#endif
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(CoreId, ChannelNumber);

#if (PWM_PARAM_CHECK == STD_ON)
        if ((Std_ReturnType)E_OK == Pwm_ValidateTriggerDelay(CoreId, PartitionChannelIdx))
        {
            if ((Std_ReturnType)E_OK == Pwm_ValidateParamTriggerDelay(CoreId, PartitionChannelIdx, TriggerDelay))
            {
#endif

                /* All validations passed. Here starts the actual functional code of the function */
                Pwm_Ipw_SetTriggerDelay(TriggerDelay, &((*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
...
#if (PWM_VALIDATE_CHANNEL_CONFIG_CALL == STD_ON)
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(CoreId, ChannelNumber, PWM_SETUCREGA_ID))
    {
#endif
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(CoreId, ChannelNumber);

        /* All validations passed. Here starts the actual functional code of the function */
        Pwm_Ipw_FastUpdateSetUCRegA((Pwm_Ipw_PeriodType)Value, &((*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
...
#if (PWM_VALIDATE_CHANNEL_CONFIG_CALL == STD_ON)
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(CoreId, ChannelNumber, PWM_SETUCREGB_ID))
    {
#endif
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(CoreId, ChannelNumber);

        /* All validations passed. Here starts the actual functional code of the function */
        Pwm_Ipw_FastUpdateSetUCRegB((Pwm_Ipw_PeriodType)Value, &((*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
...
)
{
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId(ModuleId, PWM_MASKOUTPUT_ID))
        {
            ...
            for (Index = 0 ; Index < Pwm_aState[CoreId].PwmConfig->NumInstances; Index++)
            {
                Pwm_Ipw_MaskOutputs(ModuleId, ChannelMask, UpdateType, &((*Pwm_aState[CoreId].PwmConfig->PwmInstancesConfig)[Index]));
            }
...
)
{
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId((uint8)ModuleId, PWM_UNMASKOUTPUT_ID))
        {
            ...
            for (Index = 0 ; Index < Pwm_aState[CoreId].PwmConfig->NumInstances; Index++)
            {
                Pwm_Ipw_UnMaskOutputs(ModuleId, ChannelMask, UpdateType, &((*Pwm_aState[CoreId].PwmConfig->PwmInstancesConfig)[Index]));
            }
