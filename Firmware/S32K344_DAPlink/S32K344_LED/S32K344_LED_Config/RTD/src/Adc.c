...
#define ADC_SW_MAJOR_VERSION_C                  4
...
static inline boolean Adc_ValidateTriggerLoopMode(const uint8 CoreId,
                                        ...
                                        uint8 CoreId)
{
    ...
    const Adc_HwUnitType LogicalHwUnitId  = Adc_apxCfgPtr[CoreId]->GroupsPtr[GroupIndex].AdcLogicalUnitId;
                ...
                if (ADC_INVALID_HW_GROUP_ID == OngoingHwGroupId)
                {
                    ValidState = (Std_ReturnType)E_OK;
                          ...
                          (
                            ...
                            ||
                            (Adc_apxCfgPtr[CoreId]->GroupsPtr[GroupIndex].HwTriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
                          )
                        )
                {
                                                  ...
                                                  uint8 CoreId)
{
    ...
    const Adc_HwTriggerTimerType TriggerSource = Adc_apxCfgPtr[CoreId]->GroupsPtr[GroupIndex].HwTriggerSource;
    uint8 TrigIdx = 0U;
    ...
    if (TriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
    {
         ...
         */
        (void)Adc_Ipw_MapCTUTriggerIndex(TriggerSource, CoreId, &TrigIdx);
                                                           ...
                                                           uint8 CoreId)
{
    ...
    {
        CtuPhysicalIdExpected = Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->Mapping.CtuLogToPhyId[CtuLogicalId];
        for (AdcLogicalId = 0u; AdcLogicalId < ADC_IPW_MAX_HW_UNITS_CFG; AdcLogicalId++)
        {
            if (TRUE == Adc_axUnitStatus[AdcLogicalId].CtuControlOngoing)
            {
                ...
                AdcPhysicalId = Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcLogicalId];
                                                                    ...
                                                                    const uint8 CoreId)
{
    ...
    else
    {
        ...
        CtuPhysicalIdExpected = Adc_Ipw_AdcPhysicalToCtuPhysical(Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcUnit]);
        ...
        for (AdcLogicalId = 0u; AdcLogicalId < ADC_IPW_MAX_HW_UNITS_CFG; AdcLogicalId++)
        {
            ...
            {
                /* Get CTU Instance corresponding to ADC instance */
                CtuPhysicalIdEnabled = Adc_Ipw_AdcPhysicalToCtuPhysical(Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcLogicalId]);
                /* Check if this is the expected Ctu instance */
                if (CtuPhysicalIdEnabled == CtuPhysicalIdExpected)
                {
                    CtuControlModeEnabled = TRUE; /* CtuControl Mode is considered enabled, if it is enabled for at least one AdcHwUnit which associates with that CTU */
...
{
    Std_ReturnType Status = E_OK;
    const uint8_t CtuFifoDepth[ADC_IPW_NUM_CTU_FIFO] = ADC_IPW_CTU_FIFO_DEPTH;
    ...
    else if (CtuFifoDepth[FifoIdx] < ResultLength)
    {
        Adc_ReportDetError(ServiceId, ADC_E_PARAM_CONFIG);
        Status = E_NOT_OK;
    }
#ifdef ADC_CTU_DMA_SUPPORTED
    else if (TRUE == Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->CtuConfigControlMode[CtuLogicalId]->FifoConfigs[FifoIdx].FifoDmaEnable)
    {
    ...
    if (Adc_apxCfgPtr[CoreId]->GroupsPtr[GroupIndex].TriggerSource == ADC_TRIGG_SRC_HW)
    {
        ...
        if (Adc_apxCfgPtr[CoreId]->GroupsPtr[GroupIndex].HwTriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
        {
            ...
            for (Index = 0; Index < NumberOfChannel; Index++)
            {
                ...
                PhysicalChannelId = Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId]->ChannelConfigsPtr[Channel[Index]].ChanIndex;
                                                  ...
                                                  const uint8 TriggerId)
{
    ...
    const Adc_ConfigType * const CfgPtr = Adc_apxCfgPtr[CoreId];
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_ENABLEHARDWARETRIGGER_ID, Group, CoreId))
    {
        ...
        if ((Std_ReturnType)E_OK == Adc_ValidateStateEnableHwTriggNotBusy(Group, CoreId))
        {
               ...
               )
            {
                ...
                /* Get the unit to which the group belongs to */
                LogicalHwUnitId = Adc_apxCfgPtr[CoreId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
                Adc_UpdateStatusEnableHardware(Group, LogicalHwUnitId, CoreId);
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_DISABLEHARDWARETRIGGER_ID, Group, CoreId))
    {
        ...
        if ((Std_ReturnType)E_OK == Adc_ValidateStateNotIdle(ADC_DISABLEHARDWARETRIGGER_ID, Group, CoreId))
        {
               ...
               )
            {
                ...
                /* Get the unit to which the group belongs to */
                LogicalHwUnitId = Adc_apxCfgPtr[CoreId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
                Adc_UpdateStatusDisableHardware(Group, LogicalHwUnitId, CoreId);
...
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    ReturnValue = Adc_ValidateCallAndUnit(ADC_SET_USER_GAIN_AND_OFFSET_ID, Unit, CoreId);
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */

    if ((Std_ReturnType)E_OK == ReturnValue)
    {
        ReturnValue = Adc_Ipw_SetUserGainAndOffset(Unit, CoreId, Gain, Offset);
    }
                                    ...
                                    sint16 * const CalibratedBuffer)
{
...
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    ReturnValue = Adc_ValidateCallAndUnit(ADC_APPLY_CALIBRATION_ID, Unit, CoreId);
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
    ...
    if ((Std_ReturnType)E_OK == ReturnValue)
    {
        ReturnValue = Adc_Ipw_ApplyCalibration(Unit, CoreId, BufferLength, UncalibratedBuffer, CalibratedBuffer);
    }
                    ...
                    Adc_ChannelIndexType NumberOfChannel)
{
    ...
    {
#if (ADC_CTU_HW_TRIGGER_OPTIMIZATION == STD_OFF)
        StatusChecks = Adc_CheckSetChannelCtuTriggers(GroupIndex, Channel, LogicalHwUnitId, NumberOfChannel, CoreId);
#else
...
void Adc_CtuEnableHwTrigger(Adc_CtuTrigSrcType TrigSource)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, CoreId) == (Std_ReturnType)E_OK)
    {
...
void Adc_CtuMasterReload(Adc_HwUnitType CtuUnit)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuUnit, ServiceId, CoreId) == (Std_ReturnType)E_OK)
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
...
Adc_ValueGroupType Adc_CtuReadConvData(Adc_HwUnitType AdcUnit)
{
    ...
    if (Adc_ValidateCtuControlModeApiByAdcUnit(AdcUnit, ServiceId, CoreId) == (Std_ReturnType)E_OK)
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
        AdcPhysicalId = Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcUnit];
...
void Adc_CtuStopLoopConversions(Adc_CtuTrigSrcType TrigSource)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, CoreId) == (Std_ReturnType)E_OK)
    {
        ...
        CtuPhysicalId = Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->Mapping.CtuLogToPhyId[CtuLogicalId];
        ...
        else if (FALSE == Adc_ValidateTriggerLoopMode(CoreId, CtuPhysicalId, TriggerId))
        {
                         ...
                         uint8 DataLength)
{
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, CoreId))
    {
        /* CPR_RTD_00471.adc */
        if ((Std_ReturnType)E_OK == Adc_ValidateCtuFifoParam(ServiceId, CoreId, CtuLogicalId, FifoIndex, DataLength))
        {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
                           ...
                           uint8 ResultLength)
{
    ...
    {
        /* CPR_RTD_00468.adc */
        if ((Std_ReturnType)E_OK == Adc_ValidateCtuFifoParam(ServiceId, CoreId, CtuLogicalId, FifoIndex, ResultLength))
        {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
                                  ...
                                  const uint16 TempSenseAdcConvData)
{
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_TEMPSENSE_CALCULATE_TEMP_ID, Unit, CoreId))
    {
                                    ...
                                    uint16 * const TempSenseVal)
{
    ...
    const volatile uint8 CoreId = Adc_GetCoreID();
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_TEMPSENSE_GET_TEMP_ID, Unit, CoreId))
    {
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
        if ((Std_ReturnType)E_OK == Adc_ValidateNotBusyNoQueue(Unit, ADC_TEMPSENSE_GET_TEMP_ID))
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateCheckGroupNotConversion(ADC_TEMPSENSE_GET_TEMP_ID, CoreId))
            {
                RetVal = Adc_Ipw_TempSenseGetTemp(Unit, 0u, TempSenseVal, CoreId);
