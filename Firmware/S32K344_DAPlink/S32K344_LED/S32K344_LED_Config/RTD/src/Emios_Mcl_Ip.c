...
#define EMIOS_MCL_IP_SW_MAJOR_VERSION_C                4
...
/** @implements Emios_Mcl_Ip_SetClockMode_Activity */
void Emios_Mcl_Ip_SetClockMode(uint8 Instance, const Emios_Mcl_Ip_ConfigType* const ConfigPtr, Emios_Ip_SelectPrescalerType Prescaler)
{
    ...
    for (CurrentChannel = 0U; CurrentChannel < ConfigPtr->channelsNumber; CurrentChannel++)
    {
        reg = Base->CH.UC[(*ConfigPtr->masterBusConfig)[CurrentChannel].hwChannel].C2;
        if (EMIOS_IP_PRESCALER == Prescaler)
        {
            /* Set Cn UCPRE to divider. */
            reg = ((reg & (~(eMIOS_C2_UCEXTPRE_MASK))) | (eMIOS_C2_UCEXTPRE((*ConfigPtr->masterBusConfig)[CurrentChannel].masterBusPrescaler)));
        }
        else
        {
            /* Set Cn UCPRE to divider. */
            reg = ((reg & (~(eMIOS_C2_UCEXTPRE_MASK))) | (eMIOS_C2_UCEXTPRE((*ConfigPtr->masterBusConfig)[CurrentChannel].masterBusAltPrescaler)));
        }
        Base->CH.UC[(*ConfigPtr->masterBusConfig)[CurrentChannel].hwChannel].C2 = reg;
    }
}
