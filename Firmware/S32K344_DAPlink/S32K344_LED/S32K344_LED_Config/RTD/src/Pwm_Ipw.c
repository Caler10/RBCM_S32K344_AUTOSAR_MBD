...
static uint16 Pwm_Ipw_aFlexPwmDutyCycle[FLEXPWM_IP_INSTANCE_COUNT][FLEXPWM_IP_UNIQUESUB_COUNT];
#endif
#endif
#if (PWM_FLEXIO_USED == STD_ON)
...
#if (PWM_FLEXPWM_USED == STD_ON)
#if (PWM_EMIOS_USED == STD_ON)
    if ((ModuleId >= (uint8)PWM_HW_MODULE_EMIOS_NO) && (ModuleId < (uint8)(PWM_HW_MODULE_EMIOS_NO + PWM_HW_MODULE_FLEXPWM_NO)))
#else
    if (ModuleId < PWM_HW_MODULE_FLEXPWM_NO)
#endif
    {
        ActualModuleId = ModuleId - PWM_HW_MODULE_EMIOS_NO;
        for (SubIndex = 0U; SubIndex < FLEXPWM_IP_SUBMODULE_COUNT; SubIndex++)
        {
...
Std_ReturnType  Pwm_Ipw_ValidateModuleId(uint8 ModuleId, uint8 ServiceId)
{
    ...
    {
#if (PWM_FLEXPWM_USED == STD_ON)
        if(PWM_HW_MODULE_FLEXPWM_NO <= ModuleId)
        {
            RetVal = (Std_ReturnType)E_NOT_OK;
        }
        else
    ...
    {
        if((PWM_HW_MODULE_EMIOS_NO + PWM_HW_MODULE_FLEXPWM_NO) <= ModuleId)
        {
            RetVal = (Std_ReturnType)E_NOT_OK;
        }
        else
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_FLEXPWM:
            ...
            if ((DeadTimeTicks < 0x800U) && 
                ((uint16)DeadTimeTicks < FlexPwm_Ip_aDutyCycle[IpConfig->ChannelInstanceId][UniqueSub]) &&
                (((uint16)DeadTimeTicks + FlexPwm_Ip_aDutyCycle[IpConfig->ChannelInstanceId][UniqueSub]) < FlexPwm_Ip_aPeriod[IpConfig->ChannelInstanceId][IpConfig->SubModuleId]) &&
